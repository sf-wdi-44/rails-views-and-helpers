...
<details><summary>click for explanation</summary>
  ...
  Based on the result of `@album.save`, the method will execute either the code in the `if` or in the `else`.

  If the new album `@album` correctly saves, the response will be a redirect to that album's show page. If the new album doesn't save correctly, the new album form view is rendered again. Note that neither one of these renders a `create.html.erb` file.
...
An app might have a directory `app/views/products` with the files listed below.  Explain in ~4 words what each file is for.
